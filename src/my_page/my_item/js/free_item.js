import '../css/free_item.css';
import axios from 'axios';
import cookie from 'react-cookies';
import { useEffect, useState } from 'react';

export default function Free_item(props) {
    ...
    const free_item_div_inner_footer_delete_btn=async()=> {
    ...
    const free_item_div_inner_footer_update_btn=()=> {
        for(let i = 0; i < buy_item_list.length; i++) {
            if(Number(buy_item_list[i].item_id) === Number(id)) {
    ...
    return(
        <div id='free_item_div'>
            <div id='free_item_div_inner'>
                <div id='free_item_div_inner_img'>
                    <img alt='' src={`${img}`} onClick={view_detail_item}></img>
                </div>

                <div id='free_item_div_inner_name_date'>
                    <p id='free_item_div_inner_name'>{name}</p>
                    <p id='free_item_div_inner_date'>{date}</p>
                </div>

                <div id='free_item_div_inner_content'>
                    <p>{content}</p>
                </div>

                <div id='free_item_div_inner_price'>
                    <p>{price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</p>
                </div>

                <div id='free_item_div_inner_footer'>
                    <button id='free_item_div_inner_footer_update_btn' onClick={free_item_div_inner_footer_update_btn}>수정</button>
                    <button id='free_item_div_inner_footer_delete_btn' onClick={free_item_div_inner_footer_delete_btn}>삭제</button>
                </div>
            </div>
        </div>
    )
